...
public class Main {
    ...
    public static void printIntegersWithReplace(List<Integer> list) {
        ...
        ArrayList<String> numsWords = new ArrayList<>();
        nums.add(1);
        nums.add(2);
        nums.add(3);
        numsWords.add("one");
        numsWords.add("two");
        numsWords.add("three");
        for (Integer tempListInt: list
             ) {
            int counterNumsWords = 0;
            boolean wordWritten = false;
            for (Integer tempListInt2: nums
                 ) {
                if (tempListInt2 == tempListInt){
                    System.out.println(numsWords.get(counterNumsWords));
                    wordWritten = true;
                }
                counterNumsWords++;
            }
            if (wordWritten) continue;
            System.out.println(tempListInt);
        }
    }
    //endregion
